...
export default function MicroChart(){
  ...
  useEffect(() => {
    let chart; const el = ref.current; if (!el) return;
    let tries = 0; const id = setInterval(() => {
      ...
      if (LWC && typeof LWC.createChart === "function") {
        ...
        const onR = () => chart.applyOptions({ width: el.clientWidth || 900 });
        window.addEventListener("resize", onR);
      } else if (++tries > 140) { clearInterval(id); el.innerHTML = "<div style='padding:8px;border:1px solid #a00;color:#fee;background:#210'>UMD LightweightCharts not ready</div>"; }
    }, 50);
